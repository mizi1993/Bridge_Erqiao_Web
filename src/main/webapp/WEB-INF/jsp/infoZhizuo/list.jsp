...
function __doPostBack(eventTarget, eventArgument) {
    if (!theForm.onsubmit || (theForm.onsubmit() != false)) {
		...
		<table width="98%" border="0" align="center" cellpadding="0"
			cellspacing="0">
			<tr>
				<td height="40" align="center" class="style1">支座定期巡检结果管理</td>
				<td align="right"><s:a cssClass="Button"
							action="infoZhizuo_addUI?taskId=%{#task.id}"
		...
		<div>
			<table cellspacing="0" cellpadding="3" align="Center" rules="all"
				...
				<tr class="TdTitle">
					...
					<th scope="col">支座位移等级</th>
					<th scope="col">钢件锈蚀等级</th>
					<th scope="col">混凝土松动等级</th>
					<th scope="col" style="width: 350px;">操作</th>
				</tr>
				<s:iterator value="#infoList" status="status">
					...
					<td>${weiyi_level}</td>
					<td>${xiushi_level}</td>
					<td>${songdong_level}</td>
			        <td align="center">
			        <s:a id="gridView_btnEdit_0" cssClass="Button"
						action="infoZhizuo_show?id=%{id}"
						style="display: inline-block; width: 90px;">详细&编辑</s:a> &nbsp;&nbsp;

						<s:a onclick="return confirm('你确认要删除吗');"
						id="gridView_btnDelete_0" cssClass="Button"
						action="infoZhizuo_delete?id=%{id}&&taskId=%{#task.id}"
