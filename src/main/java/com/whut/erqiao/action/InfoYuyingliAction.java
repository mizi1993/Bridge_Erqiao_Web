...
import com.whut.erqiao.entity.InfoYuyingli;
import com.whut.erqiao.entity.Task;
import com.whut.erqiao.util.ImageUploadUtil;
@Controller
@Scope("prototype")
public class InfoYuyingliAction extends BaseAction<InfoYuyingli>{

	private static final long serialVersionUID = 1L;

	public String show() {// 展示info的详细信息。
		InfoYuyingli infoYuyingli = infoYuyingliService.getById(model
				.getId());
		Task task = infoYuyingli.getTask();
		ActionContext.getContext().put("task", task);
		ActionContext.getContext().getValueStack().push(infoYuyingli);
		return "show";
	}
	...
	public String edit() {// 还要对时间日期进行转换.还要上传图片，替换掉之前的图片。
		...
		infoYuyingliService.update(model);
		if(image!=null){
	...
	public String delete() {
		infoYuyingliService.delete(model.getId());// 还要解除task与InfoYuyingli的联系.
		ActionContext.getContext().put("taskId", taskId);
		return "toTaskShow";
	}

	public String add() {// 保存infoYuyingli,并将其关联到task。还要上传图片，替换掉之前的图片。
		String date = getDateTime().substring(0, getDateTime().indexOf(" "));// 得到日期.
		String time = getDateTime().substring(getDateTime().indexOf(" ") + 1);
		model.setDate(date);
		model.setTime(time);
		...
		model.setTask(task);
		infoYuyingliService.save(model);
		if(image!=null){
			ImageUploadUtil.upload(date, newName, image,uploadDir);
		}
		ActionContext.getContext().put("taskId", taskId);
