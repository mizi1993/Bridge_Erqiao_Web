...
import com.whut.erqiao.entity.InfoXiamaotouDown;
import com.whut.erqiao.entity.Task;
import com.whut.erqiao.util.ImageUploadUtil;
@Controller
@Scope("prototype")
public class InfoXiamaotouDownAction extends BaseAction<InfoXiamaotouDown>{
	/**
	 * 
	 */
	...
	public String show() {// 展示info的详细信息。
		InfoXiamaotouDown infoXiamaotouDown = infoXiamaotouDownService.getById(model
				.getId());
		Task task = infoXiamaotouDown.getTask();
		ActionContext.getContext().put("task", task);
		ActionContext.getContext().getValueStack().push(infoXiamaotouDown);
		return "show";
	}
	...
	public String edit() {// 还要对时间日期进行转换.还要上传图片，替换掉之前的图片。
		...
		infoXiamaotouDownService.update(model);
		if(image!=null){
	...
	public String delete() {
		infoXiamaotouDownService.delete(model.getId());// 还要解除task与InfoXiamaotouDown的联系.
		ActionContext.getContext().put("taskId", taskId);
		return "toTaskShow";
	}

	public String add() {// 保存infoXiamaotouDown,并将其关联到task。还要上传图片，替换掉之前的图片。
		String date = getDateTime().substring(0, getDateTime().indexOf(" "));// 得到日期.
		String time = getDateTime().substring(getDateTime().indexOf(" ") + 1);
		model.setDate(date);
		model.setTime(time);
		...
		model.setTask(task);
		infoXiamaotouDownService.save(model);
		if(image!=null){
			ImageUploadUtil.upload(date, newName, image,uploadDir);
		}
		ActionContext.getContext().put("taskId", taskId);
